// <copyright file="Crc32HashParser.cs" company="DEMA Consulting">
// Copyright © 2018 All Rights Reserved
// </copyright>
// <author>Malcolm Nixon</author>
// <summary>Parse file for CRC32 Hash</summary>
...
{
    /// <summary>
    /// CRC32 Hash information parser
    /// </summary>
    public static class Crc32HashParser
    {
        /// <summary>
        ...
        /// Add data to a CRC32
        /// </summary>
        /// <param name="crc">Current CRC32 value</param>
        /// <param name="buf">Buffer to work on</param>
        /// <param name="count">Count of bytes</param>
        /// <returns>Updated CRC32 value</returns>
        private static uint Crc32Add(uint crc, byte[] buf, int count)
        {
        ...
        public static void Parse(FileInformation info, Options options)
        {
            try
            {
                ...
                using (var stream = File.OpenRead(info.FullPath))
                {
                    ...
                    // Loop processing chunks of the stream
                    while (true)
                    {
                        // Read the next chunk
                        ...
                        // Accumulate the bytes into the CRC
                        crc = Crc32Add(crc, buf, len);
                    }
                }
        ...
        {
            // Calculate the CRC32 of the summary
            var crc = 0xFFFFFFFFU;
            ...
            foreach (var file in files)
            {
                ...
                var hash = BitConverter.GetBytes(file.Crc32 ?? 0U);

                crc = Crc32Add(crc, name, name.Length);
                crc = Crc32Add(crc, hash, hash.Length);
            }
